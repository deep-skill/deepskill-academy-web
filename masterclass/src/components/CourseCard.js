...
const CourseCard = () => {
    ...
    return (
        <div className="w-full bg-gray-900 text-white bg-cover bg-center pb-12 md:pt-0 lg:pt-0 lg:pb-6 md:pb-6" style={{ backgroundImage: 'url("/images/background-coursecard.jpg")' }}>
            <header id="my-courses" className={`lg:bg-blue-700 lg:w-[68%] 2xl:w-[65%] lg:rounded-br-[4rem] lg:py-10 2xl:py-8 py-7 text-center md:text-left ${isMobile ? 'bg-blue-700 w-full' : ''}`}>
                            ...
                            <Carousel showThumbs={false} showStatus={false} infiniteLoop useKeyboardArrows>
                                {courses.map((course, index) => (
                                    <div key={index} className="bg-white text-gray-900 rounded-lg shadow-md relative">
                                        ...
                                        <div className="p-4">
                                            <h3 className="font-poppinsbold text-colorTextSubtitulo text-3xl mb-2 text-center">{course.title}</h3>
                                            <ul className="font-poppinsregular px-2 text-gray-700 list-disc text-lg leading-normal text-left px-6">
                                ...
                                {courses.map((course, index) => (
                                    <div key={index} className="bg-white text-gray-900 rounded-lg shadow-md flex flex-col relative lg:w-[90%] xl:w-[80%] 2xl:w-[85%]">
                                        ...
                                        <div className="px-12 lg:px-5 xl:px-8 flex flex-col flex-grow">
                                            ...
                                            <ul className="font-poppinsregular text-gray-700 list-disc text-xl lg:text-lg lg:px-6 leading-normal lg:leading-loose 2xl:leading-snug 2xl:text-base">
                                                {course.description.map((item, idx) => (
                                                    <li key={idx}>{item}</li>
                                                ))}
                                            </ul>
                                            <p className="2xl:text-lg xl:text-lg lg:text-xl my-5 font-poppinssemibold">Fecha de inicio: {course.date}</p>
                                            <div className="flex justify-center py-2 2xl:pb-2 mt-auto mb-6">
                                                <Link href={course.link}>
                                                    <button className="bg-colorsButton text-colorTextNavbar text-sm lg:text-base xl:text-base font-poppinssemibold px-14 py-3 rounded-3xl">
                                                        MÁS INFORMACIÓN AQUÍ
                                                    </button>
                                                </Link>
                                            </div>
