    ...
    return (
        <div className="w-full bg-white py-12">
            <div className="w-85% md:w-4/5 mx-auto mb-8">
                <h2 className="text-center text-32px md:text-left md:text-4xl font-poppinsbold text-colorTextSubtitulo -mb-1 ">TESTIMONIOS</h2>
                <h3 className="text-colorTextNavbar text-center text-17px md:text-left md:text-2xl font-poppinssemibold">Experiencias de nuestros ex alumnos</h3>
            </div>
                        ...
                        {testimonials.map((testimonial, index) => (
                            <div key={index} className="w-full sm:w-1/2 lg:w-[25%] p-5">
                                <div className="h-full bg-colorsTestimonial rounded-lg flex flex-col items-center text-left p-6">
                                    ...
                                    <h3 className="text-2xl font-poppinsbold text-customBlue">{testimonial.name}</h3>
                                    <p className="text-customBlue text-justify text-base font-poppinsregular my-1 ">{testimonial.role}</p>
                                    <p className="text-customBlue text-justify text-base font-poppinsregular -mt-1  ">{testimonial.country}</p>
                                    <p className="text-colorTextNavbar text-base mt-4 text-justify font-poppinsregular my-6">{testimonial.description}</p>
                                </div>
                            </div>
                        ))}
                    </div>
